...
**Email transport** is provider-agnostic (`app/notifications/email.py`) — `EMAIL_BACKEND` picks the backend: `scaleway` (Scaleway Transactional Email HTTPS API via `SCALEWAY_SECRET_KEY` + `SCALEWAY_PROJECT_ID`, region `SCALEWAY_TEM_REGION` default `fr-par`), `resend` (Resend HTTPS API via `RESEND_API_KEY`), `smtp` (stdlib `smtplib`; works with Gmail locally), or the default `console` (logs the message + link, so local dev and tests need no credentials). The two HTTPS backends both work from **Render, which blocks outbound SMTP**. Adding another provider is a ~15-line `EmailSender` subclass. `EMAIL_FROM` must be a sender the backend accepts — for Scaleway/Resend an address on a domain verified with the provider (SPF/DKIM/DMARC), e.g. `noreply@owlgraph.org`; `APP_BASE_URL` sets the origin used in the emailed links.
...
### SEC EDGAR
Imports investor data from SC 13D/13G ownership filings, executive data from Form 3/4 XML, statutory subsidiary lists from the 10-K's Exhibit 21 (`manage.py sec-ex21`), and private-company boards from Form D (`manage.py sec-formd`) — both manual-first. Controlled by `SCRAPER_SEC_EDGAR_ENABLED`. With `SEC_CACHE_DIR` set, every filing fetched is kept gzipped on disk and served from there on the next rebuild, `--force` or second pass — the EDGAR golden copy (`manage.py sec-cache stats`). With the object store configured the same filings are also kept in the shared bucket, so Render (no disk) and every other machine read one common copy.
...
| `EMAIL_BACKEND` | `` (auto) | `scaleway` \| `resend` \| `smtp` \| `console`; empty = auto (console unless `SMTP_HOST` set) |
| `RESEND_API_KEY` | — | Resend API key (for `EMAIL_BACKEND=resend`). Secret — env only |
| `SCALEWAY_SECRET_KEY` | — | Scaleway API key **Secret Key** (for `EMAIL_BACKEND=scaleway`). Secret — env only |
| `SCALEWAY_PROJECT_ID` | — | Scaleway project holding the verified TEM domain (for `EMAIL_BACKEND=scaleway`) |
| `SCALEWAY_TEM_REGION` | `fr-par` | Scaleway TEM region: `fr-par` \| `nl-ams` \| `pl-waw` |
...
| `OBJECT_STORE_BUCKET` / `_ENDPOINT` / `_ACCESS_KEY` / `_SECRET_KEY` | *(empty)* | The shared S3-compatible object store (`app/objectstore.py`) — one bucket for every environment, a prefix per purpose; `edgar/` is the filing cache's shared layer. Any S3 API (Hetzner, Scaleway, AWS); keys are secrets, env only |
...
| `mark-stale` | Mark community-tier OWNS edges unconfirmed for `--days` (180) as **stale** — dimmed in the UI, never deleted or closed. Register-tier edges and register-vouched pairs are exempt; the pass clears as well as sets. See *Source tiers* in [`docs/data-model.md`](docs/data-model.md) |
