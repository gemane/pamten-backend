...
| PATCH | `/scraper/sources/{name}/mode` | admin | Set a source's data mode: `full` draws edges, `claims_only` records claims and enriches entities but never draws structure. Does not touch existing edges |
| POST | `/scraper/sources/{name}/sweep-edges` | admin | Delete every edge a source drew (nodes and claims stay) — the retro half of claims-only. Destructive: `?confirm=` must equal the source name |
| GET | `/scraper/health` | public | Per-source health: last run/success, failure streaks, runs in 24h, bulk-dataset freshness (GLEIF publish date, PSC snapshot age) and the import lock. `last_error` and the lock `holder` are stripped outside contributor/admin — key absent, not null, same as `/scraper/runs` |
...
| POST | `/scraper/sec-13f/run` | contributor | Institutional holders of one issuer from Form 13F (~100 EDGAR fetches — filed by the holders, read one filing per manager). Quarterly by deadline: a re-run before the next 13F due date (quarter end + 45 days) returns `status: fresh` and fetches nothing; `?force=true` overrides. 409 until the SEC EDGAR scrape has stamped the entity's CIK. A refresh reads only filings newer than the last completed run (plus a week for stragglers), and holders the new quarter did not confirm are flagged `stale` (dimmed in the panel, never closed — a 13F seller files no exit, and fabricating an `until` would assert a date nobody stated) |
...
| POST | `/scraper/deduplicate-edges` | admin | Collapse duplicate active OWNS edges, keeping the largest stake (by @rid, provenance-preserving); a pair whose edges come from different sources is first folded into the survivor. Returns `{duplicates_removed, pairs_cleaned, survivors_folded}` |
