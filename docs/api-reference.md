...
| POST | `/scraper/deduplicate-entities` | admin | Merge Entity duplicates sharing an LEI / Companies House number, migrating their edges. Runs in the background by default (returns `started`; poll `GET /scraper/runs`). `background=false` runs sync in batches of `?limit=` (default 300), returning `remaining` — call until 0 |
