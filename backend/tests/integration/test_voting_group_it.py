...
class TestGroupNodeIdentity:
    def test_a_group_is_created_once(self, graph):
        ...
        gid = _upsert_voting_group("abi", "Anheuser-Busch InBev", _roster(*ABI_ROSTER),
                                   "sec", "BRC S.a R.L.")
        ...
        assert rows[0]["name"] == "Voting group — Anheuser-Busch InBev"
    ...
    def test_a_later_amendment_updates_rather_than_duplicates(self, graph):
        ...
        first = _upsert_voting_group("abi", "Anheuser-Busch InBev", _roster(*ABI_ROSTER),
                                     "sec", "BRC S.a R.L.")
        ...
        second = _upsert_voting_group("abi", "Anheuser-Busch InBev", _roster(*later),
                                      "sec", "Stichting Anheuser-Busch InBev")
    ...
    def test_the_roster_is_replaced_by_the_newer_one(self, graph):
        from app.scraper.runner import _upsert_voting_group, _upsert_owns_sec
        gid = _upsert_voting_group("abi", "ABI", _roster(*ABI_ROSTER), "sec", "BRC S.a R.L.")
        _upsert_owns_sec(gid, "abi", "sec", "unknown", "2025-01-01", None,
                         voting_power_pct=52.3)
        _upsert_voting_group("abi", "ABI", _roster(*ABI_ROSTER[:4]), "sec", "BRC S.a R.L.")
    ...
    def test_a_second_bloc_over_one_company_is_its_own_node(self, graph):
        ...
        second = _upsert_voting_group("abi", "Anheuser-Busch InBev", altria,
                                      "sec", "Altria Group, Inc.")
        assert second != first
        names = {r["name"] for r in graph.run_command(
            "MATCH (g:Entity) WHERE g.type = 'voting_group' RETURN g.name AS name")}
        assert names == {"Voting group — Anheuser-Busch InBev",
                         "Voting group — Anheuser-Busch InBev (Altria Group, Inc.)"}
    ...
    def test_the_same_roster_over_another_company_is_another_group(self, graph):
        ...
        a = _upsert_voting_group("abi", "ABI", _roster(*ABI_ROSTER), "sec", "BRC S.a R.L.")
        _upsert_owns_sec(a, "abi", "sec", "unknown", "2025-01-01", None, voting_power_pct=52.3)
        b = _upsert_voting_group("other", "Other Co", _roster(*ABI_ROSTER), "sec", "BRC S.a R.L.")
        assert a != b
