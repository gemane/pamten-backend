...
def _register_verified_user(client) -> str:
    ...
    with patch("app.auth.router.send_verification_email",
        ...
        client.post("/auth/register", json={"email": "mfa@x.com", "password": "password123"})
    client.post("/auth/verify-email", json={"token": captured["v"]})
    r = client.post("/auth/login", json={"email": "mfa@x.com", "password": "password123"})
...
def test_enable_then_login_requires_totp_and_recovery_code(client):
    ...
    mfa_token = client.post("/auth/login", json={"email": "mfa@x.com", "password": "password123"}).json()["mfa_token"]
