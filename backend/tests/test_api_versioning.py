...
is not cosmetic: federation peers call a hardcoded `{base_url}/federation/export`
and a released mobile app pins whatever path it shipped with, so dropping it
breaks callers we don't deploy. These tests are what should fail if someone
"tidies up" by deleting one of the two mounts.
"""
...
def test_the_schema_documents_the_real_api(client):
    paths = _documented_paths(client)
    for expected in ("/v1/search/", "/v1/auth/login", "/v1/entities/", "/v1/federation/export"):
...
def test_legacy_unversioned_paths_still_serve(client):
    ...
    for legacy in ("/auth/users", "/federation/export", "/flags/"):
...
def test_versioned_and_legacy_paths_behave_the_same(client):
    for path in ("/auth/users", "/federation/export"):
...
def test_legacy_paths_are_hidden_from_the_schema(client):
    ...
    assert "/federation/export" not in paths
